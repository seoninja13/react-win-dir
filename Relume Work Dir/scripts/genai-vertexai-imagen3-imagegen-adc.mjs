/**
 * Vertex AI Image Generation with Application Default Credentials (ADC)
 * 
 ...
 * 
 * Usage:
 *   node scripts/genai-vertexai-imagen3-imagegen-adc.mjs \
...
class VertexAIClient {
  ...
  validateEnvironment() {
    if (!process.env.GOOGLE_CLOUD_PROJECT) {
      throw new Error('GOOGLE_CLOUD_PROJECT environment variable is required for ADC authentication');
    }
  }
  ...
  initializeClient() {
    try {
      ...
      console.log(`Initialized Vertex AI client with ADC for project: ${process.env.GOOGLE_CLOUD_PROJECT}`);
    } catch (error) {
