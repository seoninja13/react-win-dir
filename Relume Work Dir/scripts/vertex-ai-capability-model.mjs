/**
 * Vertex AI Image Generation using imagen-3.0-capability model
 * Based on quota dashboard showing this model is available
 */

console.log('🚀 Testing imagen-3.0-capability model...');
...
async function generateImage() {
  try {
    ...
    const outputDir = outputArg ? outputArg.split('=')[1].replace(/"/g, '') : 'generated-images/capability-test';
    
    console.log('🎨 Prompt:', prompt);
    console.log('📁 Output directory:', outputDir);
    
    // Configuration using imagen-3.0-capability (from quota dashboard)
    const CONFIG = {
      projectId: process.env.GOOGLE_CLOUD_PROJECT,
      location: process.env.GOOGLE_CLOUD_LOCATION,
      model: 'imagen-3.0-capability', // Using the model with 10 req/min quota
    };
    ...
    // Initialize the generative model with imagen-3.0-capability
    console.log('🤖 Initializing imagen-3.0-capability model...');
    ...
    console.log('📊 Using model with 10 requests/minute quota');
      ...
      if (candidate.content && candidate.content.parts && candidate.content.parts[0]) {
        ...
        if (part.text) {
          ...
          try {
            ...
            const filename = `imagen-capability-${timestamp}.png`;
