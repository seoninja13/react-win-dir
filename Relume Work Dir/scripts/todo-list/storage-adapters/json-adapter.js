/**
 * JSON Storage Adapter for To-Do List Management
 * 
 * Handles storing and retrieving tasks in JSON format.
 */
...
/**
 * JSON Storage Adapter class
 */
class JsonStorageAdapter {
  /**
   * Create a new JsonStorageAdapter instance
  ...
  async createBackupIfNeeded() {
    ...
    try {
      const backupFileName = `tasks_backup_${new Date().toISOString().replace(/:/g, '-')}.json`;
  ...
  cleanupOldBackups() {
    try {
      const backupFiles = fs.readdirSync(this.backupPath)
        .filter(file => file.startsWith('tasks_backup_') && file.endsWith('.json'))
        .map(file => ({
          name: file,
