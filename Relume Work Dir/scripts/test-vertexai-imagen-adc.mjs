/**
 * Test script for Vertex AI Imagen with Application Default Credentials (ADC)
 * 
 * This script tests the ADC authentication implementation
 * for generating images using Google's Vertex AI Imagen 3.0 model.
 * 
 * Prerequisites:
 * - GOOGLE_CLOUD_PROJECT environment variable set to a valid GCP project ID
 * - Application Default Credentials configured (gcloud auth application-default login)
 * 
 * Usage:
 *   node scripts/test-vertexai-imagen-adc.mjs
 */

import { generateImage, generateBatchImages } from './genai-vertexai-imagen3-imagegen-adc.mjs';
...
const TEST_OUTPUT_DIR = path.join(__dirname, '..', 'public', 'test-images', 'adc');
...
async function testSingleImageGeneration() {
  console.log('\n=== Testing Single Image Generation (ADC) ===');
...
async function testBatchImageGeneration() {
  console.log('\n=== Testing Batch Image Generation (ADC) ===');
...
async function runTests() {
  console.log('=== Vertex AI Imagen ADC Authentication Tests ===');
