...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error(
      "❌ Error in API route /admin/api/vertex-ai/image/generate:",
      error
    );
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Google Cloud authentication or project configuration error. Check server logs and environment variables (GOOGLE_APPLICATION_CREDENTIALS, GOOGLE_CLOUD_PROJECT, GOOGLE_CLOUD_LOCATION).",
    ...
    if (
      error.name === "VertexAIImageServiceError" ||
      error.name === "VertexAIError"
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    return NextResponse.json(
      {
        ...
        error:
          error.message ||
          "Failed to generate image due to an internal server error.",
