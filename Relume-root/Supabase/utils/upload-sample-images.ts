...
async function processImageDirectory(
  ...
  console.log(
    `Processing directory: ${sourceDirectoryPath} for category: ${imageCategory}`
  );
  console.log(`Target Supabase bucket: ${targetBucketName}`);

  try {
    // Ensure the target bucket exists
    await createBucketIfNotExists(targetBucketName, { public: true });
    ...
    for (const file of files) {
      ...
      if (stat.isFile()) {
        ...
        if (!contentType) {
          console.log(
            `Skipping non-image or unsupported file: ${fullFilePath}`
          );
          skippedCount++;
          continue;
        }
        ...
        try {
          ...
          const filePathInBucket = path.join(
            imageCategory,
            path.basename(fullFilePath)
          );

          console.log(
            `Uploading ${fullFilePath} to ${targetBucketName}/${filePathInBucket}...`
          );
    ...
    console.log(
      `Finished processing ${imageCategory}. Uploaded: ${uploadedCount}, Skipped: ${skippedCount}`
    );
  } catch (error) {
...
async function uploadAllSampleImages() {
  ...
  if (fs.existsSync(windowsSourceDir)) {
    ...
    console.warn(
      `Windows sample images directory not found: ${windowsSourceDir}`
    );
  }
