...
import { uploadImage, createBucketIfNotExists, GENERATED_IMAGES_BUCKET } from './storage.js';
  ...
  try {
    ...
    for (const entry of records) {
      ...
      try {
        ...
        const fileName = entry.target_url.replace(/^\//, '').replace(/\//g, '-') + '.png';
        ...
        // Upload to Supabase
        const supabaseUrl = await uploadImage(imageResponse.imageUrl, filePath, GENERATED_IMAGES_BUCKET, metadata);
        ...
        console.log(`Successfully generated and uploaded image: ${supabaseUrl}`);
    ...
    console.log(`Batch processing complete. ${successCount}/${results.length} images generated successfully.`);
...
(async () => {
  try {
    ...
    console.error('Error during batch image generation script execution:', error);
