...
export function initializeGenAIClient() {
  if (GOOGLE_GENAI_USE_VERTEXAI) {
    if (!GOOGLE_CLOUD_PROJECT) {
      throw new Error('GOOGLE_CLOUD_PROJECT environment variable is required when using Vertex AI');
    }
 ...
 */
export async function generateImage(prompt: string, model = 'gemini-2.0-flash-preview-image-generation') {
  const ai = initializeGenAIClient();
