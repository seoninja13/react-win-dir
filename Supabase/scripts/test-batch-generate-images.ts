...
async function log(message: string, details?: any) {
  ...
  fs.appendFileSync(LOG_PATH, logMessage + (details ? ` ${JSON.stringify(details)}` : '') + '\n');
...
async function processTestBatch() {
  ...
  await logger.info('Starting test batch processing with batch size', { batchSize: BATCH_SIZE });
  ...
  try {
    ...
    if (!fs.existsSync(PROCESSED_DATA_PATH)) {
      await log('Error: Processed data not found. Please run process-image-csv.ts first.');
      await logger.error('Processed data not found', { path: PROCESSED_DATA_PATH });
      return;
    }
  
    // Read processed data
    const processedData: ProcessedData = JSON.parse(fs.readFileSync(PROCESSED_DATA_PATH, 'utf8'));
    await log(`Loaded processed data with ${processedData.stats.totalImages} images`);
    await logger.info('Loaded processed data', { totalImages: processedData.stats.totalImages });
    ...
    for (let i = 0; i < testBatch.length; i++) {
      ...
      await log(`Processing image ${i + 1}/${testBatch.length}: ${item.imageType} for ${item.originalUrl}`);
    ...
    try {
      ...
      if (!imageData || !imageData.imageUrl) {
        ...
        await logger.error('No image was generated', { imageType: item.imageType, originalUrl: item.originalUrl });
        throw error;
      }
      
      await logger.info('Image generated successfully', { enhancedPrompt: imageData.enhancedPrompt ? 'present' : 'not present' });
      
      // Save image to Supabase storage
      await logger.info('Saving image to Supabase storage', { imagePath: item.newImagePath });
      const imagePath = await saveImageToSupabase(imageData.imageUrl, item.newImagePath);
      ...
      await logger.debug('Adding delay between requests to avoid rate limiting', { delayMs: 2000 });
    ...
    await log(`Test batch processing completed. Results saved to ${RESULTS_PATH}`);
    await logger.info('Test batch processing completed', { resultsPath: RESULTS_PATH });
    
    // Summary
    const successCount = results.filter(r => r.success).length;
    await log(`Summary: ${successCount}/${results.length} images generated successfully`);
 ...
 * Save image to Supabase storage
 */
async function saveImageToSupabase(imageUrl: string, imagePath: string): Promise<string> {
  try {
    ...
    await logger.debug('Converted base64 to buffer', { bufferSize: buffer.length });
    
    // Ensure the path starts with a slash
    const normalizedPath = imagePath.startsWith('/') ? imagePath.substring(1) : imagePath;
    
    // Upload to Supabase
    await logger.info('Uploading to Supabase storage', { bucket: 'images', path: normalizedPath });
    ...
    const err = error instanceof Error ? error : new Error(String(error));
    await logErrorObject(err, 'test-batch-generate-images', { stage: 'save-to-supabase', imagePath });
    throw err;
  }
}
...
processTestBatch().catch(async error => {
  ...
  await logErrorObject(err, 'test-batch-generate-images', { stage: 'script-execution' });
  process.exit(1);
});
